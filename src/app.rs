use crate::downloader::{DownloadEvent, DownloadHandler, DownloaderOutcome, manifest_audio};
use crate::{
    cli::Cli,
...
impl AppState {
    ...
    pub fn tick(&mut self) {
        ...
        // 2. Now that the handler borrow is over, update the library
        if let Some(path) = path_to_add {
            // Assuming your Library has an add_file method
            self.library.add_file(&path).unwrap();
        }
    }
